...
# Cố định đường dẫn file VIP luôn nằm cùng thư mục với bot
...
async def resolve_uid(session: aiohttp.ClientSession, input_val: str) -> str | None:
    """Hỗ trợ phân tích đa dạng URL, ID và Username"""
    input_val = input_val.strip()

    # 1. Xử lý link rút gọn dạng locket.camera/links/...
    # Cần gửi request để đi theo (follow redirect) lấy được link gốc chứa mã mời
    if "locket.camera/links/" in input_val:
        try:
            ...
            pass # Bỏ qua nếu lỗi mạng để nó chuyển xuống fallback bên dưới

    # 2. Xử lý link mời dạng locket.camera/invites/...
    # Firebase UID luôn có độ dài cố định là 28 ký tự. Locket nối UID với một đoạn hash phía sau.
    if "locket.camera/invites/" in input_val:
        try:
            # Tách chuỗi để lấy phần mã sau chữ invites/ (VD: 5KvAasAfUJPDnOcGcf1wUuxInKq15fedb39b4a570fcb)
            invite_part = input_val.split("locket.camera/invites/")[1].split("?")[0].split("/")[0]
            if len(invite_part) >= 28:
                return invite_part[:28] # Cắt đúng 28 ký tự đầu tiên để lấy Firebase UID gốc
        except Exception:
            pass

    # 3. Tiền xử lý cho định dạng Username và locket.cam/...
    text = input_val.replace("https://", "").replace("http://", "")
    # Bỏ đi query parameters (?...) nếu vô tình copy dính vào
    clean = text.lstrip("@").strip().split("?")[0]
    
    # Nếu người dùng nhập chuẩn 1 UID (28 ký tự)
    if len(clean) == 28 and " " not in clean and "/" not in clean:
        return clean

    # Nếu người dùng nhập dạng locket.cam/username
    if "locket.cam/" in clean:
        username = clean.rstrip("/").split("/")[-1]
    else:
        username = clean

    # 4. Fallback cuối cùng: Nhờ API bên thứ 3 phân giải Username (vì user ID không nằm trong text)
...
async def main():
    ...
    print("\nChọn loại gói Gold bạn muốn share:")
    ...
    if not vips:
        ...
        print(f"👉 Tool đã tạo file '{file_name}' ở cùng thư mục. Vui lòng mở file, nhập danh sách Username/UID VIP (mỗi dòng 1 tài khoản) rồi mở lại tool.")
        return
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            # Cập nhật lời nhắc nhập liệu để người dùng biết có thể dán cả Link
